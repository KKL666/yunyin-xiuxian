...
function t(
  ...
  desc: string,
  opts: { icon?: string; fixedMods?: StatMods } = {}
): EquipmentTemplate {
  ...
  return { id, name, slot, minTier, base, desc, icon: opts.icon ?? defaultIcon[slot], fixedMods: opts.fixedMods }
}

export const EQUIPMENT_TEMPLATES: EquipmentTemplate[] = [
  // ---- 武器(攻击为主) ----
  t('w_zhuqing', '青竹剑', 'weapon', 1, { attack: 10 }, '山后砍来的竹子削成,胜在趁手'),
  t('w_xuantie', '玄铁重剑', 'weapon', 2, { attack: 12 }, '玄铁铸就,大巧不工'),
  ...
  t('h_xuantie', '玄铁冠', 'head', 4, { defense: 5, maxHp: 20 }, '沉重却牢靠'),
  t('h_xingchen', '星辰冠', 'head', 8, { defense: 6, maxHp: 24 }, '嵌有陨星碎屑,夜里微光流动', { fixedMods: { qiRegen: 0.06 } }),
  ...
  t('b_xuanwu', '玄武甲', 'body', 6, { defense: 10, maxHp: 42 }, '仿玄武之甲铸成,厚重难破', { fixedMods: { damageReduction: 0.04 } }),
  t('b_liuyun', '流云仙衣', 'body', 10, { defense: 11, maxHp: 48 }, '轻若流云,水火不侵', { fixedMods: { dodgeRate: 0.03 } }),
  t('b_xingluo', '星罗法衣', 'body', 14, { defense: 12, maxHp: 55 }, '衣上星图与周天同转', { fixedMods: { qiRegen: 0.08 } }),
